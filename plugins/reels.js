...
Asena.addCommand({pattern: 'instagram ?(.*)', fromMe: false, desc: "Downloads from instagaram" , dontAddCommandList: true }, async (message, match) => {
   ...
   const { status, type, data } = await instagram(match[1], 'julie')
    ...
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: "*Made by LisaMwol*", quoted: message.data })
    
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: "*Made by LisaMwol*", quoted: message.data })
